function createPoint(){
  ...
  function nums2arrs(num){
    const arr=[]
    arr[2] = num2arr(num%10)
    arr[1] = num2arr(num%100/10|0)
    arr[0] = num2arr(num%1000/100|0)
    return arr
  }
  ...
  function horizontal90(nums){
    return nums.map(item=>{
      ...
      return arrs2nums([[c1,b1,a1],[c2,b2,a2],[c3,b3,a3]])
    })
  }
  ...
  function vertical90(arrs){
    const res = [[],[],[]]
    const [b1,s1,g1] = num2arr(arrs[0])
    const [b2,s2,g2] = num2arr(arrs[1])
    const [b3,s3,g3] = num2arr(arrs[2])
    return [b3*100+b2*10+b1,s3*100+s2*10+s1,g3*100+g2*10+g1]
  }
  ...
  function points2arrs(points){
    return points.map(items=>{
      ...
      items.forEach(e=>e<2000?(item[0]+=e):e<3000?(item[1]+=e):(item[2]+=e))
      return item
